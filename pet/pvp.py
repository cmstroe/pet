...
class FundingPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [text,'. Does the previous text talk about a funding round?', self.mask], []
        elif self.pattern_id == 1:
            return [text, 'Does the previous sentence talk about an invesment?', self.mask], []
        elif self.pattern_id == 2:
            return [text, ' Did the company get an investment?', self.mask], []
        elif self.pattern_id == 3:
            return [text, ' Did the company raise money?', self.mask], []
        elif self.pattern_id == 4:
            return [text, ' Did an investment happen?', self.mask], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
...
class   MAPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            return [text,'. Did an acquisition happen?', self.mask], []
        elif self.pattern_id == 1:
            return [text, 'Did the company get acquired?', self.mask], []
        elif self.pattern_id == 2:
            return [text, 'Did a company acquire another?', self.mask], []
        elif self.pattern_id == 3:
            return [text, 'Was there a merger between companies?', self.mask], []
        elif self.pattern_id == 4:
            return [text, 'Did the companies merge?', self.mask], []
        elif self.pattern_id == 5:
            return [text, 'Was there a corporate event?', self.mask], []
